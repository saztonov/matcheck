-- УПД-очередь: поля, индексы, ослабленный check, таблица llm_calls, новый промпт.
--
-- Должна идти отдельной миграцией ПОСЛЕ 0015, потому что использует значения
-- enum source_status ('queued', 'processing'), добавленные в 0015, а PG не
-- разрешает использовать новые значения enum в той же транзакции, в которой
-- они были добавлены (см. ошибку 55P04 «unsafe use of new value»).
--
...
-- 1) Новые поля в source_documents для очереди и журнала ошибок.
...
-- 2) Идемпотентность повторной загрузки одного и того же PDF (тот же
...
-- 3) Быстрый поиск незавершённых джобов для recovery после рестарта воркера.
CREATE INDEX "source_documents_unfinished_idx"
  ON "source_documents" ("status", "parsed_at")
  WHERE "status" IN ('queued', 'processing');
--> statement-breakpoint

-- 4) Ослабить старый check: для statuses queued/processing/needs_resolution/
...
-- 5) Журнал общения с LLM. Хранит сырой запрос и ответ провайдера, чтобы
...
-- 6) Обновлённый промпт УПД (v2). Старая версия деактивируется (уникальный
