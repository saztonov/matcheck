-- УПД-промпт v3: явное требование вернуть confidence в каждом ответе.
--
-- В v2 в схеме confidence не был required, и LLM системно возвращала
-- ответ без этого поля. Zod.default(0.5) подставлял 0.5, из-за чего у
-- всех PDF-УПД в UI колонка «Уверенность» = 50% (см. реальный лог
-- llm_calls для УПД 201/21125720 от 13.05.2026).
--
-- В этой миграции добавлен раздел «# Поле confidence» с явной шкалой,
-- и подчёркнуто, что поле обязательное. Параллельно в коде:
--   - apps/api/src/domain/edo/upd-pdf.parser.ts → required: ['items', 'confidence'].
--   - packages/contracts/src/source-documents.ts → confidence без .default(0.5).
-- Если LLM по-прежнему не вернёт confidence — Zod упадёт, воркер
-- пометит документ parse_failed (это лучше, чем тихая 50% «уверенность»).

UPDATE "prompts" SET "is_active" = false WHERE "doc_kind" = 'upd';
--> statement-breakpoint
INSERT INTO "prompts" ("doc_kind", "name", "content", "is_active") VALUES
  ('upd', 'default v3', $PROMPT_UPD_V3$Ты извлекаешь данные из текста российского УПД (универсального передаточного документа), полученного через распознавание PDF.
...
- Поля `items` и `confidence` ОБЯЗАТЕЛЬНЫ. Без них ответ невалиден.$PROMPT_UPD_V3$, true);
